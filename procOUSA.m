...
if cumulative
    ...
    labels = { ...
        ...
        'COG serv.'; ...
        'Proj. Conn.'};
    depts = [100;101;102;103;120;150;200; ...
             300;350;370;400;500;550; ...
             600;700;800;850;900];
...
% Budget Processing
    ...
    for i = 1:n
        ...
        if length(str) >= 7
            ...
            % further divide admin based on CO
            if data.dept(i) == 100
                switch data.cobj(i)
                    case 400
                        data.dept(i) = 101; % Membership Income
                    case 513
                        data.dept(i) = 101; % Membership Postage
                    case 548
                        data.dept(i) = 101; % Membership Fulfillment 
                    case 401
                        data.dept(i) = 102; % Fundraising Donations
                    case 546
                        data.dept(i) = 102; % Fundraising Expenses
                    case 504
                        data.dept(i) = 103; % Employee Salary
                    case 505
                        data.dept(i) = 103; % Employee Insurance
                    case 555
                        data.dept(i) = 103; % Employee Paychex
                end
            end
        end
    end
...
% Transaction Processing
    ...
    for i = 1:n
        if ~isnan(dates(i))
            if ~isnan(debit(i)) || ~isnan(crdit(i))
                ...
                str = cll{1};
                if length(str) >= 7 && ...
                   deptMap.isKey(str2double(str(5:7)))
                    good(i) = 1;
                    if ~isnan(debit(i))
                        amnt(i) = -debit(i);
    ...
    for i = 1:n
        ...
        if strcmp(newdept{i},'Admin')
            switch cobj{i}
                case 400
                    newdept{i} = deptMap(101); % Membership Income
                case 513
                    newdept{i} = deptMap(101); % Membership Postage
                case 548
                    newdept{i} = deptMap(101); % Membership Fulfillment
                case 401
                    newdept{i} = deptMap(102); % Fundraising Donations
                case 546
                    newdept{i} = deptMap(102); % Fundraising Expenses
                case 504
                    newdept{i} = deptMap(103); % Employee Salary
                case 505
                    newdept{i} = deptMap(103); % Employee Insurance
                case 555
                    newdept{i} = deptMap(103); % Employee Paychex
            end
        end
